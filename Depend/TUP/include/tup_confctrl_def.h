...
typedef struct tagTE_CONF_CTRL_INFO
{
    ...
    TUP_UINT8            ucConfState;                                           /**< [en]Indicates conference state value refer to CONFCTRL_E_CONF_STATE. [cn]会控状态(CONF_STATE)，取值参考CONFCTRL_E_CONF_STATE */
...
typedef struct tagCONFCTRL_S_BOOK_CONF_INFO_UPORTAL
{
    ...
    TUP_CHAR start_time[CONFCTRL_D_MAX_TIME_FORMATE_LEN];  /**< [en]Indicates conference start time, format:YYYY-MM-DD HH:MM  ,this param is optional. [cn]可选，会议开始时间，格式：YYYY-MM-DD HH:MM，如果没有指定开始时间，则表示会议马上开始 */
